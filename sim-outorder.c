...
compute_issue_priority(struct RUU_station *rs)
{
  ...
  /* Slight bonus if all inputs are ready and it's queued (should be true here) */
  if (rs->queued) p += 1;

  /* You can add more signals if you like, e.g., critical path approximations */
...
ruu_issue(void)
{
  ...
  for (n_issued=0;
    ...
    /* count priority top issues */
    if (!strcmp(issue_policy, "priority") && n_issued == 0)
        sim_priority_issued++;

    /* existing SimpleScalar issue checks... */
